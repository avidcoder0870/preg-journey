...
public class HealthCheckService {
	...
	public HealthCheck performHealthCheck(HealthCheckRequest request, Long performedByUserId) {
		...
		// If there is an existing health check ID provided, we check if it exists and
		// belongs to the same patient. If it does, we update that record instead of
		// creating a new one.
		if (request.getId() != null) {
			log.info("Health check ID provided in request: {}. Checking if it exists for patient: {} ", request.getId(),
					patient.getId());
			existingHealthCheckOpt = healthCheckRepository.findById(request.getId());
		}
		if (existingHealthCheckOpt.isPresent()) {
			log.info("Existing health check found for patient: {} on date: {}. Updating existing record. id {} ",
					patient.getMotherId(), request.getCheckDate(), existingHealthCheckOpt.get().getId());
			...
			healthCheck = HealthCheck.builder().patient(patient)
					.checkDate(request.getCheckDate() != null ? request.getCheckDate() : LocalDate.now())
					.bpSystolic(request.getBpSystolic()).bpDiastolic(request.getBpDiastolic())
					.pulseRate(request.getPulseRate()).temperature(request.getTemperature())
					.respiratoryRate(request.getRespiratoryRate()).spo2(request.getSpo2())
					.hemoglobin(request.getHemoglobin()).bloodSugarFasting(request.getBloodSugarFasting())
					.bloodSugarPP(request.getBloodSugarPP()).bloodSugarRandom(request.getBloodSugarRandom())
					.weight(request.getWeight()).height(request.getHeight()).fundalHeight(request.getFundalHeight())
					.fetalHeartRate(request.getFetalHeartRate()).fetalMovement(request.getFetalMovement())
					.urineAlbumin(request.getUrineAlbumin()).urineSugar(request.getUrineSugar())
					.symptoms(request.getSymptoms()).swellingObserved(request.getSwellingObserved())
					.bleedingReported(request.getBleedingReported()).headacheReported(request.getHeadacheReported())
					.blurredVisionReported(request.getBlurredVisionReported())
					.abdominalPainReported(request.getAbdominalPainReported()).notes(request.getNotes())
					.recommendations(request.getRecommendations()).nextCheckDate(request.getNextCheckDate())
					.performedBy(performedBy).build();
		}

		// Perform risk assessment
		RiskAssessmentService.RiskAssessmentResult riskResult = riskAssessmentService.assessRisk(healthCheck, patient);
		...
		if (Boolean.TRUE.equals(request.getScheduleFollowUp()) && request.getFollowUpDate() != null) {
			...
			if (request.getFollowUpAssigneeId() != null) {
				assignee = userRepository.findById(request.getFollowUpAssigneeId()).orElse(performedBy);
			} else {
			...
			log.info("Manual follow-up scheduled for patient: {} on {}", patient.getMotherId(),
					request.getFollowUpDate());
		}

		// Auto follow-up: Based on risk level (if not manually scheduled and
		// auto-enabled)
		if (!manualFollowUpScheduled && !Boolean.FALSE.equals(request.getAutoFollowUpEnabled())) {
			if (riskResult.riskLevel() == RiskLevel.RED) {
				...
				createFollowUp(savedHealthCheck, patient, performedBy, followUpDate,
						"Auto-scheduled follow-up for HIGH RISK patient. Risk factors: "
								+ String.join(", ", riskResult.riskFactors()));
				log.info("Auto follow-up scheduled for RED risk patient: {} on {}", patient.getMotherId(),
						followUpDate);
				...
				createFollowUp(savedHealthCheck, patient, performedBy, followUpDate,
						"Auto-scheduled follow-up for MODERATE RISK patient. Risk factors: "
								+ String.join(", ", riskResult.riskFactors()));
				log.info("Auto follow-up scheduled for YELLOW risk patient: {} on {}", patient.getMotherId(),
						followUpDate);
			}
		}

		log.info("Health check completed. Risk Level: {}, Score: {}", riskResult.riskLevel(), riskResult.score());

		return savedHealthCheck;
	}

	private void createFollowUp(HealthCheck healthCheck, Patient patient, User assignedTo, LocalDate scheduledDate,
			String notes) {
		FollowUp followUp = FollowUp.builder().patient(patient).assignedTo(assignedTo).scheduledDate(scheduledDate)
				.status(FollowUpStatus.PENDING).triggeredByHealthCheck(healthCheck).notes(notes).build();
	...
	private void createRiskAlert(HealthCheck healthCheck, Patient patient,
		...
		String title = riskResult.riskLevel() == RiskLevel.RED ? "CRITICAL: High Risk Patient Detected"
		...
		RiskAlert alert = RiskAlert.builder().patient(patient).healthCheck(healthCheck)
				.alertType(AlertType.HIGH_RISK_DETECTED).severity(riskResult.riskLevel()).title(title)
				.description(description).riskFactors(String.join("; ", riskResult.riskFactors()))
				.recommendedAction(recommendedAction).build();
	...
	public void deleteHealthCheck(Long id) {
		...
		log.info("Deleting health check with id: {} for patient: {}", healthCheck.getId(),
				healthCheck.getPatient().getMotherId());
		// delete associated risk alerts
		riskAlertRepository.deleteByHealthCheck(healthCheck);

		// delete associated follow-ups
		log.info("Deleting follow-ups triggered by health check id: {}", healthCheck.getId());
		followUpRepository.deleteByTriggeredByHealthCheck(healthCheck);

		// delete the health check
		log.info("Deleting health check record with id: {}", healthCheck.getId());
		healthCheckRepository.delete(healthCheck);

		// mark patient risk as GREEN and let it be recalculated on next health check
		log.info("Resetting patient risk level to GREEN for patient: {} after health check deletion",
				healthCheck.getPatient().getMotherId());
		patientService.updatePatientRisk(healthCheck.getPatient().getId(), 0, RiskLevel.GREEN);
	}
}
